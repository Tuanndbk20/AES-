...
private static final int[] Tinv0 =
    ...
    */
    ...
    private int[][] generateWorkingKey(byte[] key, boolean forEncryption)
    {
        ...
        int KC = keyLen >>> 2; //// KC=8
        ROUNDS = KC + 6;  // This is not always true for the generalized Rijndael that allows larger block sizes
        System.out.println("number of round keys: "+ROUNDS);
        ...
        case 4:
        {
            ...
            for (int i = 1; i <= 10; ++i)
            {
                int u = subWord(shift(t3, 8)) ^ rcon[i - 1]; // u <=> ti
                t0 ^= u;  W[i][0] = t0;
                t1 ^= t0; W[i][1] = t1;
                t2 ^= t1; W[i][2] = t2;
                t3 ^= t2; W[i][3] = t3;
        ...
        case 8:
        {
        	System.out.println("calculate t0 -> t7(process 32 byte = 4 word Key)!");
            int t0 = Pack.littleEndianToInt(key,  0); W[0][0] = t0;
            int t1 = Pack.littleEndianToInt(key,  4); W[0][1] = t1;
            int t2 = Pack.littleEndianToInt(key,  8); W[0][2] = t2;
            int t3 = Pack.littleEndianToInt(key, 12); W[0][3] = t3;
        ...
        for(int i=0; i<(ROUNDS+1) ; i++) {
        	for(int j=0; j<4;j++) {
        		System.out.print(toHex(Pack.intToLittleEndian(W[i][j]))+" ");
        	}
        	System.out.println();
        }
    ...
    public AESEngine()
    {
    	System.out.println("initialize AES");
    }
     ...
     * @param keyParameter the parameters required to set up the cipher.
        ...
        KeyParameter  keyParameter)
    {
        ...
        {
        	System.out.println("generate Round Keys!");
            WorkingKey = generateWorkingKey(((KeyParameter)keyParameter).getKey(), forEncryption);
        ...
        throw new IllegalArgumentException("invalid parameter passed to AES init - " + keyParameter.getClass().getName());
    }
    ...
    {
    	System.out.println("AddRoundKey for plaintext(W0->W3 xor C0->C3)!");
        int t0 = this.C0 ^ KW[0][0];
        int t1 = this.C1 ^ KW[0][1];
        int t2 = this.C2 ^ KW[0][2];

        int r = 1, r0, r1, r2, r3 = this.C3 ^ KW[0][3];
        System.out.println("C0: "+toHex(Pack.intToLittleEndian(C0)));
        System.out.println("C1: "+toHex(Pack.intToLittleEndian(C1)));
        System.out.println("C2: "+toHex(Pack.intToLittleEndian(C2)));
        System.out.println("C3: "+toHex(Pack.intToLittleEndian(C3)));
        while (r < ROUNDS - 1)
        {
            r0 = T0[t0&255] ^ shift(T0[(t1>>8)&255], 24) ^ shift(T0[(t2>>16)&255], 16) ^ shift(T0[(r3>>24)&255], 8) ^ KW[r][0];
            System.out.println("T0[t0&255]: "+toHex(Pack.intToLittleEndian(T0[t0&255])));
            System.out.println("t0&255: "+(t0&255));
            System.out.println("t1: "+t1);
            System.out.println("t1>>8: "+(t1>>8));
            System.out.println("(t1>>8)&255): "+((t1>>8)&255));
            System.out.println("T0[(t1>>8)&255]: "+(T0[(t1>>8)&255]));
            System.out.println("shift(T0[(t1>>8)&255], 24): "+toHex(Pack.intToLittleEndian(shift(T0[(t1>>8)&255], 24))));
